...
TEST(unit_arcfile, decode_arcfile_keys_nominal)
{
	...
	i32_num_tokens = JSON_read_value(JSON_KEY_ARCFILE_KEY_CONTENT, JSON_VALUE_TYPE_STRING, pc_json, pc_buffer);

	TEST_ASSERT_EQUAL(9, i32_num_tokens);
	TEST_ASSERT_EQUAL_STRING("text_0.txt", pc_buffer);

	JSON_read_value(JSON_KEY_ARCFILE_KEY_CHOICES, JSON_VALUE_TYPE_STRING, pc_json, pc_buffer);
