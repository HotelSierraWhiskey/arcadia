...
#ifndef BOOTLOADER
#define NVMCTRL_LOG_DBG(fmt, ...)   		SHELL_printf("\r%-12s" fmt, "[NVMCTRL]", ##__VA_ARGS__)
#define NVMCTRL_LOG_WARN(fmt, ...)   		SHELL_PRINT_WARNING("\r%-12s" fmt, "[NVMCTRL]", ##__VA_ARGS__)
# endif // !BOOTLOADER
 ...
 */
volatile uint8_t * ppu8_app_nvm_rows[NVMCTRL_APP_NVM_ROW_NUM_ROWS];
...
{
	uintptr_t app_nvm_rows_start = (uintptr_t)&app_nvm_base;

	// Initialize app_nvm section row pointers
	ppu8_app_nvm_rows[NVMCTRL_APP_NVM_ROW_ID_0] = (volatile uint8_t *)app_nvm_rows_start + (0 * NVMCTRL_NVM_ROW_SIZE);
	ppu8_app_nvm_rows[NVMCTRL_APP_NVM_ROW_ID_1] = (volatile uint8_t *)app_nvm_rows_start + (1 * NVMCTRL_NVM_ROW_SIZE);
	ppu8_app_nvm_rows[NVMCTRL_APP_NVM_ROW_ID_2] = (volatile uint8_t *)app_nvm_rows_start + (2 * NVMCTRL_NVM_ROW_SIZE);
	ppu8_app_nvm_rows[NVMCTRL_APP_NVM_ROW_ID_3] = (volatile uint8_t *)app_nvm_rows_start + (3 * NVMCTRL_NVM_ROW_SIZE);
...
static void NVMCTRL_exec(uint8_t command)
{
	if (command == NVMCTRL_COMMAND_ERASE_ROW || 
